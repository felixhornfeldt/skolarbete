...
# Function is working and gives correct result
# Updated version with one minor change
def documented_functions(arr)
    ...
    begin
        ...
        while i < arr.length
            ...
            temp_holder = arr[i]
            while j < temp_holder.length
                if temp_holder[j] == "#" && temp_holder.length > 2 # the minor change that makes the function work (temp_holder.length > 2)
